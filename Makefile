# This Makefile is for the github_creator extension to perl.
#
...
#     ABSTRACT => q[Create a Github repository for your Perl module]
#     AUTHOR => q[brian d foy <bdfoy@cpan.org>]
#     EXE_FILES => [q[github_creator]]
#     LICENSE => q[perl]
#     MAN1PODS => { github_creator=>q[$(INST_MAN1DIR)/github_creator.1] }
#     NAME => q[github_creator]
#     PREREQ_PM => { Config::IniFiles=>q[0], Test::More=>q[0], YAML=>q[0], WWW::Mechanize=>q[0], File::Find::Closures=>q[0] }
#     VERSION_FROM => q[github_creator]
...
MAKE = make
FULLEXT = github_creator
BASEEXT = github_creator
PARENT_NAME = 
DLBASE = $(BASEEXT)
...
DISTNAME = github_creator
DISTVNAME = github_creator-0.1
...
pure_all :: $(INST_SCRIPT)/github_creator
	$(NOECHO) $(NOOP)

realclean ::
	$(RM_F) \
	  $(INST_SCRIPT)/github_creator 

$(INST_SCRIPT)/github_creator : github_creator $(FIRST_MAKEFILE) $(INST_SCRIPT)$(DFSEP).exists $(INST_BIN)$(DFSEP).exists
	$(NOECHO) $(RM_F) $(INST_SCRIPT)/github_creator
	$(CP) github_creator $(INST_SCRIPT)/github_creator
	$(FIXIN) $(INST_SCRIPT)/github_creator
	-$(NOECHO) $(CHMOD) $(PERM_RWX) $(INST_SCRIPT)/github_creator
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                github_creator' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.1' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Create a Github repository for your Perl module' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    File::Find::Closures:          0' >> META_new.yml
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Create a Github repository for your Perl module</ABSTRACT>' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="File-Find-Closures" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
